...
class ClassificationTree:
    ...
    def buildTree(self, D, features, featureTypes, level):
        ...
        print "Best split feature: ", (f, s), " Data split: (left, right) = ", (len(left), len(right))
    ...
    def findBestSplitOnFeature(self, D, f, isNominal):
        sortedD = self.sortData(D, f)
        splits = self.findAllSplitPoint(sortedD, f, isNominal)
        #print "total splits ", len(splits)
        H_D = self.entropy(sortedD, self.l, self.labels)
        maxIA = 0.0
        bestSplit = None
        for split, i in splits:
            left = sortedD[:i]
            right = sortedD[i:]
            #left, right = self.splitData(sortedD, f, isNominal, split)

            H_left = ((len(left) * 1.0) / len(sortedD)) * self.entropy(left, self.l, self.labels)
            H_right = ((len(right) * 1.0) / len(sortedD)) * self.entropy(right, self.l, self.labels)
            H_A = H_left + H_right
            I_A = H_D - H_A
